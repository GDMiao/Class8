...
//		OctetsStream& operator << (unsigned long x)      { return push_byte(byteorder_32(x)); }
		OctetsStream& operator << (aint64 x)            { return push_byte(byteorder_64(x)); }
		OctetsStream& operator << (float x)              { return push_byte(byteorder_32(*(unsigned int*)&x)); }
		OctetsStream& operator << (double x)             { return push_byte(byteorder_64(*(auint64*)&x)); }
		...
		OctetsStream& push_byte(const char *x, auint32 len)
		{
			insert(end(), x, len);
			return *this;
		}
...
//			return *this;
//		}
		const OctetsStream& operator >> (const aint64 &x) const
		{
			remove_const(x) = pop_byte_64();
			return *this;
		}
		//FIX ME by linzihan
		//const OctetsStream& operator >> (const CECID64 &x) const
		//{
		//	remove_const(x) = pop_byte_64();
		//	return *this;
		//}
		...
		void pop_byte(char *x, auint32 len) const
		{
			if (pos + len > size()) throw Marshal::Exception();
		...
		Octets& replace(const void *data, auint32 size)
		{
			pos = 0;
