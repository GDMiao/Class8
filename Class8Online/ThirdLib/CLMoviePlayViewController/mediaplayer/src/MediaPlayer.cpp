...
MPLATER_API bool   AVP_Play(const char* szPlayUrl,int nPlayStreamType,PlayAddress arrAddress[4],int narrAddressNum, HIBITMAP hBGbitmap)
{
	...
	if(rp)
	{
		...
		for(int i = 0;i<narrAddressNum;i++)
		{
			...
			rp->PlayMedia(arrAddress[i].szPushAddr,arrAddress[i].nMediaType,arrAddress[i].hwnd,hBGbitmap,bIsPlay,arrAddress[i].nUserID,arrAddress[i].bIsMainVideo,arrAddress[i].bIsStudent,arrAddress[i].bIsVideShow);
		}
	}
	return true; 
}
...
MPLATER_API bool   AVP_Change(const char* szPlayUrl,int nPlayStreamType,PlayAddress arrAddress[4],int narrAddressNum,HIBITMAP hBGbitmap)
{
	...
	if((rp = (CPlayProcess*)FindPlayUnit(szVideoURL,szAudioURL)))
	{
		for(int i = 0;i<narrAddressNum;i++)
		{
			...
			if(!rp->ChangeMedia(arrAddress[i].szPushAddr,arrAddress[i].nMediaType,arrAddress[i].hwnd,hBGbitmap,bIsPlay,arrAddress[i].nUserID,arrAddress[i].bIsMainVideo,arrAddress[i].bIsStudent,arrAddress[i].bIsVideShow))
			{
				return false;
			}
		}
...
MPLATER_API bool  playFileSwitch(const char* szCurPlayLocalFileName)
{
	...
	for(iter = m_glistPlayUnit.begin(); iter != m_glistPlayUnit.end();iter++)
	{
		...
		if(pu.pPlayProcess && rpTemp->getIsPlayFile())
		{
			...
			{
				rpTemp->PauseFile(szCurPlayLocalFileName,true);
				rpTemp->SwitchPaly(szCurPlayLocalFileName);
			}
		}
	}
	m_gPlayUnitLock.Unlock();
