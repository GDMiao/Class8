...
#define Default_GIF_EMOJI_W 25  //默认表情宽度
#define Default_GIF_EMOJI_H 25  //默认表情高度

#define Default_NET_IMG_W 92    //默认网络图片宽度
#define Default_NET_IMG_H 80    //默认网络图片宽度
		 ...
		 [(__bridge NSArray*)CTLineGetGlyphRuns((__bridge CTLineRef)(line)) enumerateObjectsUsingBlock:^(id run, NSUInteger index, BOOL* stop)
		  {
			  ...
			  else if (attributes[@"img"])
			  {
                  ...
                  if (attributes[@"image"]) {
                      ...
                      //获取最后图片名
                      NSString *imgName = [attributes[@"src"] lastPathComponent];
                      if ([imgName rangeOfString:@".gif"].location != NSNotFound) {
                          //GIF表情图片
                          ...
                          imageView.start = YES;
                          imageView.frame  = CGRectMake(bounds.origin.x, self.bounds.size.height-(bounds.origin.y+bounds.size.height)+2, bounds.size.width, bounds.size.height);;
                          [self addSubview:imageView];
                      }else {
                          //网络图片
                          ChatImageView *imageView = [[ChatImageView alloc] initWithFrame:CGRectMake(bounds.origin.x, self.bounds.size.height-(bounds.origin.y+bounds.size.height)+2, bounds.size.width, bounds.size.height)];
                          [imageView setImageWithUrl:[NSString stringWithFormat:@"%@%@",strDownloadChatImageUrl,imgName] placeholderImage:self.placeholderImage];
                          [self addSubview:imageView];
                      }
                  }
                  ...
                  if (attributes[@"image"]) {
                      CGContextDrawImage(context, bounds, [attributes[@"image"] CGImage]);
                  }else if (attributes[@"gif"])
                  {
                      //GIF表情图片
                      
                      NSString *filePath = [[NSBundle mainBundle] pathForResource:attributes[@"src"] ofType:nil];
                      NSData *data = [[NSData alloc] initWithContentsOfFile:filePath];
                      SCGIFImageView *imageView = [[SCGIFImageView alloc] initWithGIFData:data];
                      imageView.start = YES;
                      imageView.frame  = CGRectMake(bounds.origin.x, self.bounds.size.height-(bounds.origin.y+bounds.size.height)+2, bounds.size.width, bounds.size.height);;
                      ...
                      //下载图片
                      ChatImageView *imageView = [[ChatImageView alloc] initWithFrame:CGRectMake(bounds.origin.x, self.bounds.size.height-(bounds.origin.y+bounds.size.height)+2, bounds.size.width, bounds.size.height)];
                      [imageView setImageWithUrl:attributes[@"src"] placeholderImage:self.placeholderImage];
//                      [imageView sd_setImageWithURL:[NSURL URLWithString:attributes[@"src"]] placeholderImage:self.placeholderImage options:SDWebImageRetryFailed];
                      [self addSubview:imageView];
...
-(void)doLink:(UIGestureRecognizer*)gesture
{
	...
	[m_links enumerateKeysAndObjectsUsingBlock:^(NSValue* key, id url, BOOL* stop)
	{
	...
	if ((self=[super init])!=nil)
	{
		...
		struct callbacks
		{
			static void startElement(HTMLParser* parser, const xmlChar* name,const xmlChar** atts)
			{
				...
				{
                    if (parser.linkColor) {
                        [parser->m_style.lastObject setValue:(id)parser.linkColor.CGColor forKey:(id)kCTForegroundColorAttributeName];
                    }else{
                        [parser->m_style.lastObject setValue:(id)[UIColor blueColor].CGColor forKey:(id)kCTForegroundColorAttributeName];
                    }
					
                    if (parser.showUnderline) {
                        [parser->m_style.lastObject setValue:@(kCTUnderlineStyleSingle) forKey:(id)kCTUnderlineStyleAttributeName];
                    }
...
//									[parser->m_style.lastObject setValue:[NSURL URLWithString:[NSString stringWithUTF8String:(const char*)value]] forKey:@"src"];
                            ...
                            [parser->m_style.lastObject setValue:[NSString stringWithUTF8String:(const char*)value] forKey:@"src"];
                            [parser->m_style.lastObject setValue:@"img" forKey:@"img"];
							[parser->m_style.lastObject setValue:image forKey:@"image"];
						}
                        else if (xmlStrcasecmp(key,BAD_CAST"src")==0 && xmlStrcasecmp(name,BAD_CAST"GIF")==0){
                            //匹配GIF
                            [parser->m_style.lastObject setValue:[NSString stringWithUTF8String:(const char*)value] forKey:@"src"];
                            [parser->m_style.lastObject setValue:@"img" forKey:@"img"];
                            [parser->m_style.lastObject setValue:[NSString stringWithUTF8String:(const char*)value] forKey:@"gif"];
						...
						else if (xmlStrcasecmp(key,BAD_CAST"href")==0)
						{
                            ...
                            [parser->m_style.lastObject setValue:href forKey:@"href"];
...
//							[parser->m_style.lastObject setValue:url forKey:@"href"];
				...
				for (NSString* key in [parser->m_style.lastObject allKeys])
				{
					...
					{
						parser->m_paragraph[paragraph].spec=kCTParagraphStyleSpecifierLineSpacingAdjustment;
						CGFloat size=[parser->m_style.lastObject[@"linespace"] floatValue];
						parser->m_paragraph[paragraph].value=&size;
						parser->m_paragraph[paragraph].valueSize=sizeof(CGFloat);
						++paragraph;
					}
				}
				...
				else if (xmlStrcasecmp(name,BAD_CAST"img")==0 || xmlStrcasecmp(name,BAD_CAST"GIF")==0)
				{
						...
						static CGFloat width(void* ref)
						{
							...
							if (width==nil)
							{
                                if (style[@"image"]) {
                                    ...
                                    NSString *tmpImgName = style[@"src"];
                                    if ([tmpImgName.lastPathComponent rangeOfString:@".gif"].location != NSNotFound) {
                                        //GIF 表情
                                        width = @(Default_GIF_EMOJI_W);
                                    }else if ([tmpImgName.lastPathComponent rangeOfString:@".jpg"].location != NSNotFound) {
                                        width = @(Default_NET_IMG_W);
                                    }else{
                                        width= @(40);
                                    }
                                }
						...
						static CGFloat height(void* ref)
						{
							...
							if (height==nil)
							{
                                if (style[@"image"]) {
                                    ...
                                    if ([tmpImgName.lastPathComponent rangeOfString:@".gif"].location != NSNotFound) {
                                        //GIF 表情
                                        height = @(Default_GIF_EMOJI_H);
                                    }else if ([tmpImgName.lastPathComponent rangeOfString:@".jpg"].location != NSNotFound) {
                                        height = @(Default_NET_IMG_H);
                                    }else{
                                        height= @(40);
                                    }
                                }
...
//			[m_style setArray:@[@{@"font":@"Helvetica",@"size":@([UIFont systemFontSize])}]];
            ...
            //字号
            if (self.textFont) {
                [dic setObject:@(self.textFont.pointSize) forKey:@"size"];
            }else{
                [dic setObject:@([UIFont systemFontSize]) forKey:@"size"];
            }
            
            //颜色
            if (self.textColor) {
                [dic setValue:(id)self.textColor.CGColor forKey:(id)kCTForegroundColorAttributeName];
            }else{
                [dic setObject:(id)[UIColor blackColor].CGColor forKey:(id)kCTForegroundColorAttributeName];
            }
			//行间距
			[dic setObject:@(4) forKey:@"linespace"];

            [m_style addObject:dic];
...
//			{
                ...
                if (!IOS6_OR_LATER) {
                    ...
                    NSRange range = NSMakeRange(0,[m_attributedString length]);
                    [m_attributedString removeAttribute:(NSString*)(kCTStrokeWidthAttributeName) range:range];
                    [m_attributedString addAttribute:(NSString*)(kCTStrokeWidthAttributeName) value:(__bridge id)strokeWidth range:range];
                    [m_attributedString addAttribute:(NSString*)(kCTStrokeColorAttributeName) value:(id)[[UIColor whiteColor] CGColor] range:range];
...
+(NSAttributedString*)attributedStringWithHTML:(NSString*)html renderer:(id<HTMLRenderer> (^)(NSMutableDictionary* attributes))renderer
{
    ...
    NSString *tmpHtml;
    if ([html hasPrefix:@"<html><body>"]) {
        tmpHtml = html;
    }else{
        tmpHtml = [NSString stringWithFormat:@"<html><body>%@</body></html>",html];
    }
	return [parser parse:tmpHtml];
}
...
{
    HTMLParser* parser=[[HTMLParser alloc] init];
    parser.textColor=textColor;
    parser.textFont = textFont;
    parser.linkColor = linkColor;
    NSString *tmpHtml;
    if ([html hasPrefix:@"<html><body>"]) {
        tmpHtml = html;
    }else{
    ...
    NSAttributedString *tmpAttributedString =[parser parse:tmpHtml];
