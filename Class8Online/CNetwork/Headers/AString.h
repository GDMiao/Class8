...
public:		//	Operations
	...
	A_FORCEINLINE char operator [] (int n) const { ASSERT(n >= 0 && n <= GetLength()); return m_pStr[n]; }
	char& operator [] (int n);

	A_FORCEINLINE operator const char* () const { return m_pStr; }

	//	Get string length
	A_FORCEINLINE int	GetLength()	const { return GetData()->iDataLen; }
	//	String is empty ?
	A_FORCEINLINE bool IsEmpty() const { return m_pStr == m_pEmptyStr ? true : false; }
	//	Empty string
	A_FORCEINLINE void Empty() { FreeBuffer(GetData()); m_pStr = m_pEmptyStr; }
	...
	auint64 ToInt64() const;
	//	Convert string to double
	inline double ToDouble() const;

	//util func 
	static abool	IsNumeric(const char* szStr);
	static aint32	FindChar(const achar* szStr, achar c);
