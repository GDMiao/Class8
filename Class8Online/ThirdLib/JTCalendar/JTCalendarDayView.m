...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if(selected){
        if (self.calendarManager.calendarAppearance.firstLoad) {
            self.calendarManager.calendarAppearance.firstLoad = NO;
        ...
        if(!self.isOtherMonth){
            ...
            if ([self.calendarManager.calendarAppearance firstLoad]) {
                ...
                textLabel.textColor = [self.calendarManager.calendarAppearance dayCircleColorToday];
        ...
        if(!self.isOtherMonth){
            ...
            if (self.isweekDay){
                //周末的文字颜色
                textLabel.textColor = [self.calendarManager.calendarAppearance dayTextColorWeekDay];
                dotView.color = [self.calendarManager.calendarAppearance dayDotColorTodayOtherMonth];
            }else{
                //除了周末及未选中及当日的其他全部天的文字颜色
...
- (BOOL)isweekDay
{
    NSInteger selfTag = self.tag;
    if (self.date) {
        if (selfTag == 1 || selfTag == 7) {
            return YES;
        }
    }
    return NO;
