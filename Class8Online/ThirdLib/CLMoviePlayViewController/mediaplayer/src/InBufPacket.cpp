...
CInBuffer* CInBufPacket::GetBusyPacket(struct timeval nLastPacketTime,unsigned int nDiffPts)
{
	...
	else
	{
		if(m_bIsFirst || nDiffPts > m_nMaxWaitTime)
		{
			if(m_mapInPacket.size() > 0)
			{
                ...
                unsigned int nLastPacketTimeEx = timeVar.tv_sec;
				...
				{
					if(m_mapInPacketIter->second->getTimePts() < nLastPacketTimeEx)
					{
                        if(m_mapInPacketIter->second->getTimePts() >= m_nLastRecvTimePts)
                        {
                            nLastPacketTimeEx = m_mapInPacketIter->second->getTimePts();
...
void CInBufPacket::putBufToInPacket(unsigned char*recvbuf,unsigned int nSize)
{
	...
	if(pInBuf)
	{
		...
		{
            if(m_bIsFirstRecvPacket)
            {
                m_recvLastPacketNo = nCurRecvPacketNo;
                m_bIsFirstRecvPacket = false;
            }
            else
            {
                bool bIsPush = true;
...
int CInBufPacket::CheckGiveUPPacket(unsigned int nSSCD,struct timeval nCurSysTime,char szBuf[256])
{
    int nSize = 0;
    if(m_giveUPPacket.size() > 0)
    {
        ...
        while(mapGiveUPIter != m_giveUPPacket.end())
        {
            if(mapGiveUPIter->second.bIsTomeOut(nCurSysTime, m_nMaxWaitTime))
            {
                m_giveUPPacket.erase(mapGiveUPIter++);
            }
            else
            {
                ...
                {
                    int nPacketNp = mapGiveUPIter->first;
                    if(bIsFirst)
                    {
                    ...
                    if(nSize < 256)
                    {
                        sprintf(szBuf,"%s|%d" ,szBuf,nPacketNp);
                    }
                    nSize = strlen(szBuf) + 1;
                }
                mapGiveUPIter++;
