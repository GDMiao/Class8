...
class Compress
{
	...
	unsigned char* compress_block( unsigned char *obuf, auint32 isize )
	{
...
public:
	...
	Octets& Update(Octets &in)
	{
		...
		unsigned int isize = in.size();
		unsigned int remain = MPPC_HIST_LEN - ( histptr - history ) - legacy_in;
		if ( isize >= remain )
		{
