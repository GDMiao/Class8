...
CUDPClinet::CUDPClinet(unsigned int nDestIP,unsigned short nPDestPort,const char* szStreamID)
{
	...
	m_recvSocket = -1;
	m_nRTPPayloadType = 0;
	m_readLastPacketTime.tv_sec = 0;
        m_readLastPacketTime.tv_usec = 0;
	m_SedLosePacketTime.tv_sec = 0;
        m_SedLosePacketTime.tv_usec = 0;
	m_statTime = 0;
	...
	m_lastGetLiveTime.tv_sec = 0;
        m_lastGetLiveTime.tv_usec = 0;
	m_bIsReHello = false;
...
CUDPClinet::CUDPClinet(const char* szDestIP,unsigned short nPDestPort,const char* szStreamID)
{
	...
	m_nDestPort = nPDestPort;
    inet_pton(AF_INET, m_szDestIP, (void *)&m_nDestIP);
...
int  CUDPClinet::Groupsock(unsigned short nPort)
{
    ...
    if(bind(newSocket, ((const struct sockaddr*)(&sockaddr)), len) < 0)
    {
        close(newSocket);
        return 0;
    }
...
bool CUDPClinet::InitUDPClient(int nType,long nUserID)
{
	m_nType = nType;
    if(nType == UDPPULL)
	    m_nUserID = nUserID;
	...
	//unsigned int nIP = inet_addr(m_szDestIP);
            unsigned int nIP = 0;
            inet_pton(AF_INET, m_szDestIP, (void *)&nIP);
	if(m_sendSocket > 0)
	{
...
{
    struct timeval nCurTimePts;
    gettimeofday(&nCurTimePts,NULL);
	unsigned int nDiff = (nCurTimePts.tv_sec - m_readLastPacketTime.tv_sec)*1000 + (nCurTimePts.tv_usec - m_readLastPacketTime.tv_usec)/1000;
...
void CUDPClinet::recv_loop()
{
	while(!m_recvThread.GetStop())
	{
		...
		{
                     struct timeval nCurKPTime;
            gettimeofday(&nCurKPTime, NULL);
            if(m_bIsLogin)
            {
                int nDiff = (nCurKPTime.tv_sec - m_lastGetLiveTime.tv_sec);
                if(nDiff > m_nMaxKeepAliveWaitTime)
                {
                    m_bIsLogin = false;
                    m_lastGetLiveTime = nCurKPTime;
                    m_InBufPacket.ResetInBuffer();
                }
            }
            ...
            char szMsg[256] = {'\0'};
            char szTempMsg[256] = {'0'};
            if(m_InBufPacket.CheckGiveUPPacket(m_nSSRC, nCurSysTime,szMsg) > 0)
            {
                COutBuffer * pOutSendBuf = m_OutBufPacket.buildPacket((unsigned char*)szMsg,strlen(szMsg)+1,1,0,true);
                //unsigned int nIP = inet_addr(m_szDestIP);
                unsigned int nIP = 0;
                inet_pton(AF_INET, m_szDestIP, (void *)&nIP);
                if(pOutSendBuf && m_recvSocket > 0)
                {
                    MAKE_SOCKADDR_IN(dest,nIP,htons(m_nDestPort+1));
                    socklen_t addlen = sizeof(dest);
                    sendto(m_recvSocket,(char*)pOutSendBuf->startPtr(),pOutSendBuf->curPacketSize(),0,(const sockaddr*)&dest,addlen);
                }
                m_OutBufPacket.PutFreeOutBuffer(pOutSendBuf);
		...
		if(m_recvSocket > 0 )
		{
			...
			if(m_bIsLogin)
			{
				int nDiff = (nCurSysTime.tv_sec  - m_lastGetLiveTime.tv_sec)/1000;
