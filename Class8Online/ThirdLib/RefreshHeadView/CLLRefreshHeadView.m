...
{
    if (!_statusLabel) {
        _statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(_circleView.right + 5 ,15, 160, 14)];
...
{
    if (!_circleView) {
        _circleView = [[CLLRefreshCircleView alloc] initWithFrame:CGRectMake((SCREENWIDTH - 80 - CLLRefreshCircleViewHeight) * 0.5,15,CLLRefreshCircleViewHeight,CLLRefreshCircleViewHeight)];
    }
    return _circleView;
}
