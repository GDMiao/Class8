...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.clearanceIconWitTitle = 5.0f;
...
- (void)commonInit {
    ...
    self.badgeIndexArray = [NSMutableArray arrayWithCapacity:1];
    self.badgeImage = [UIImage imageNamed:@"对话留言"];
...
- (CGSize)measureTitleAtIndex:(NSInteger )index
{
    id titleString = self.sectionTitles[index];
    CGFloat stringWidth = 0;
    CGFloat stringHeight = 0;
    if([titleString respondsToSelector:@selector(sizeWithAttributes:)]) {
        stringWidth = [titleString sizeWithAttributes:@{NSFontAttributeName: self.textFont}].width;
        stringHeight = [titleString sizeWithAttributes:@{NSFontAttributeName: self.textFont}].height;
    }
    else {
        stringWidth = roundf([titleString sizeWithFont:self.textFont].width);
        stringHeight = roundf([titleString sizeWithFont:self.textFont].height);
    }
...
- (NSString *)attributedTitleAtIndex:(NSInteger )index {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            titleLayer.font = (__bridge CFTypeRef)(self.textFont.fontName);
            titleLayer.fontSize = self.textFont.pointSize;
            ...
            //显示badge
            if ([self.badgeIndexArray containsObject:[NSNumber numberWithInt:idx]] && self.badgeImage) {
                CGFloat imageWidth = self.badgeImage.size.width;
                CGFloat imageHeight = self.badgeImage.size.height;
                CALayer *imageLayer = [CALayer layer];
                imageLayer.frame = CGRectMake(rect.origin.x + rect.size.width+2, rect.origin.y + rect.size.height/2 - imageHeight/2, imageWidth, imageHeight);
                
                imageLayer.contents = (id)self.badgeImage.CGImage;
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat stringHeight = textSize.height;
			CGFloat yOffset = roundf(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight - stringHeight) * 0.5));
            ...
            CGRect rectDiv;
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                imageXOffset = (self.segmentWidth * idx) + (self.segmentWidth - textWidth - self.clearanceIconWitTitle - imageWidth) * 0.5;
                textXOffset = imageXOffset + self.clearanceIconWitTitle + imageWidth;
                rectDiv = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), self.selectionIndicatorHeight * 2, self.verticalDividerWidth, self.frame.size.height - (self.selectionIndicatorHeight * 4));
            } else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
                CGFloat xOffset = 0;
                ...
                imageXOffset = xOffset + ([self.segmentWidthsArray[idx] floatValue] - imageWidth - self.clearanceIconWitTitle - textWidth) * 0.5; //(self.segmentWidth / 2.0f) - (imageWidth / 2.0f)
                textXOffset = imageXOffset + imageWidth + self.clearanceIconWitTitle;
                rectDiv = CGRectMake(xOffset - (self.verticalDividerWidth / 2), self.selectionIndicatorHeight * 2, self.verticalDividerWidth, self.frame.size.height - (self.selectionIndicatorHeight * 4));
            }
            
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight - imageHeight) * 0.5);
            ...
            if ([self.badgeIndexArray containsObject:[NSNumber numberWithInt:idx]] && self.badgeImage) {
                ...
                CGFloat imageHeight = self.badgeImage.size.height;
                CALayer *imageLayer = [CALayer layer];
                imageLayer.frame = CGRectMake(imageRect.origin.x + imageRect.size.width, imageRect.origin.y - 2, imageWidth, imageHeight);
...
- (void)updateSegmentsRects {
    ...
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            self.segmentWidth = MAX(stringWidth + image.size.width + self.clearanceIconWitTitle, self.segmentWidth);
        }];
    } else if (self.type == HMSegmentedControlTypeTextImages && HMSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
        
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.right + self.segmentEdgeInset.left;
            UIImage *sectionImage = [self.sectionImages objectAtIndex:idx];
            CGFloat imageWidth = sectionImage.size.width;

            
            CGFloat combinedWidth = imageWidth + stringWidth + self.clearanceIconWitTitle;
...
{
    [self.badgeIndexArray addObject:[NSNumber numberWithInt:index]];
    [self setNeedsDisplay];
}
-(void)hiddenBadge:(int)index
{
    [self.badgeIndexArray removeObject:[NSNumber numberWithInt:index]];
    [self setNeedsDisplay];
}
 ...
 * 更新选项快文字
 **/
- (void)upDateTitle:(NSString *)text itemAtIndex:(int)index
{
    ...
    for (int i = 0; i < self.sectionTitles.count; i ++) {
        if (index == i) {
            ...
            [titles addObject:[self.sectionTitles objectAtIndex:i]];
        }
    }
