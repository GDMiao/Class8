...
- (void) show:(iToastType) type{
	...
	if (image) {
		...
		v.frame = CGRectMake(0, 0, textSize.width + 25, textSize.height + 20);
...
//    v.layer.borderColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.5].CGColor;
//    
//    v.layer.borderWidth = 2.0;
    
	UIWindow *window = nil;
    for (UIWindow *item in [[UIApplication sharedApplication] windows]) {
        if ([[item.class description] isEqualToString:@"UITextEffectsWindow"]) {
            window = item;
            break;
        }
    }
	...
	switch (orientation) {
		case UIDeviceOrientationPortrait:
		{
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(window.frame.size.width / 2, window.frame.size.height - 55 - v.frame.size.height);
		...
		case UIDeviceOrientationPortraitUpsideDown:
		{
			...
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(width / 2, 55 + v.frame.size.height);
			} else if (theSettings.gravity == iToastGravityCenter) {
		...
		case UIDeviceOrientationLandscapeLeft:
		{
			...
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(55 + v.frame.size.width,window.frame.size.height / 2);
			} else if (theSettings.gravity == iToastGravityCenter) {
				point = CGPointMake(window.frame.size.width/2, window.frame.size.height/2);
			} else {
				// TODO : handle this case
		...
		case UIDeviceOrientationLandscapeRight:
		{
			...
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(window.frame.size.width - 55 - v.frame.size.width, window.frame.size.height/2);
			} else if (theSettings.gravity == iToastGravityCenter) {
				point = CGPointMake(window.frame.size.width/2, window.frame.size.height/2);
			} else {
				// TODO : handle this case
